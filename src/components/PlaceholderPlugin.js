...
const placeholder = `import {CloudinaryImage} from "@cloudinary/url-gen";
  ...
  export default function App() {
      const cldImage = new CloudinaryImage("rose-peach", { cloudName: "picturecloud7" });
      // if no modeType is specified "blur" is the default
      const modeType = "pixelate" 
      return (
          <div>
          <h3>Render a Placeholder Image</h3>
          <h4>Image with {modeType} Placeholder </h4>
          <AdvancedImage width="40%" cldImg={cldImage} plugins={[placeholder({mode: modeType})]} />
      </div>
      )
  }`;
...
<AdvancedImage width="40%" cldImg={rose-peach} plugins={[placeholder({mode: modeType})]}/>`}
